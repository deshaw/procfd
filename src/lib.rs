...
const WRITE: FDPermissions = FDPermissions::WRITE;
...
fn process2fdtargets(
    ...
    if fd_filter.query_fds() {
        for fd in &process.fds {
            let mode = if fd.mode() & (READ | WRITE) == (READ | WRITE) {
                String::from("rw")
            } else if fd.mode() & WRITE == WRITE {
                String::from("w")
            } else if fd.mode() & READ == READ {
                String::from("r")
            } else {
                String::new()
            };
...
/// Get file descriptor entries based on the provided arguments
///
/// # Errors
/// Returns an error if there's an issue collecting network maps or socket peer data
pub fn get_fd_entries(args: &Args) -> Result<Vec<FDEntry>> {
